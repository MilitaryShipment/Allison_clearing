...
class ClearingDaemon{
    ...
    public function __construct(){
        echo "New Instance Constructed\n";
        $voicent = new \Voicent();
        $this->_getShipments();
        echo count($this->shipments) . " shipments found\n";
        foreach($this->shipments as $shipment){
            ...
            if(!$this->_hasBeenSent($shipment->Reference,$primaryPhone)){
                //no call has ever been made. Call primary for first time.
            ...
            }elseif(LogReader::callMade($primaryPhone) && LogReader::wasAnsweredByHuman($primaryPhone) && !count(LogReader::lastCallResults($primaryPhone)) && $this->_getCallCount($primaryPhone) < 2){
                echo "A Call was made, A human Answered and didn't answer any questions\n";
                echo "CALL AGAIN\n";
                $voicent->clearing_call($primaryPhone,$this->_cleanScac($shipment->Carrier),$this->_buildAddressString($shipment),$shipment->DestinationCity,$this->_getFullState($shipment->DestinationState),$shipment->DestinationZip,$shipment->AssignedTo,"");
                $this->_createNotification($shipment,$primaryPhone);
            }elseif(LogReader::callMade($primaryPhone) && !LogReader::wasAnsweredByHuman($primaryPhone) && !$this->_hasBeenSent($shipment->Reference,$secondaryPhone)){
                ...
                echo "A Call was made, and a machine answered.";
                echo "CALL SECONDARY PHONE";
            }elseif(LogReader::callMade($primaryPhone) && LogReader::wasAnsweredByHuman($primaryPhone) && count(LogReader::lastCallResults($primaryPhone)) > 1){
                echo "A call was made, a human answered and DID answer questions\n";
                echo "This shipment is complete";
                continue;
            }
        }
    }
    ...
    protected function _getCallCount($phoneNumber){
        $count = 0;
        $results = $GLOBALS['db']
            ->suite('mssql')
            ->driver('mssql')
            ->database('Sandbox')
            ->table('ctl_notifications')
            ->select('count(*) as count')
            ->where("message_to","=",$phoneNumber)
            ->get();
        while($row = sqlsrv_fetch_array($results,SQLSRV_FETCH_ASSOC)){
            $count = $row['count'];
        }
